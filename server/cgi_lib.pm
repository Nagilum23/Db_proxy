...
package cgi_lib;
$cgi_lib'version = sprintf("%d.%02d", q$Revision: 2.3 $ =~ /(\d+)\.(\d+)/);


# Parameters affecting cgi_lib behavior
# User-configurable parameters affecting file upload.
$cgi_lib'maxdata    = 16777216;    # maximum bytes to accept via POST - 2^24
$cgi_lib'writefiles =      0;    # directory to which to write files, or
                                 # 0 if files should not be written
$cgi_lib'filepre    = "cgi_lib"; # Prefix of file names, in directory above
...
sub ReadParse {
  ...
  if ($len > $cgi_lib'maxdata) { #'
      &CgiDie("cgi_lib.pm: Request to receive too much data: $len bytes\n");
  }
  ...
  if (!defined $meth || $meth eq '' || $meth eq 'GET' || 
    ...
    if (!defined $meth || $meth eq '') {
      ...
      &CgiDie("cgi_lib.pm: Unknown request method: $meth\n");
    }
   ...
   PART: # find each part of the multi-part while reading data
    while (1) {
     ...
     BODY: 
	  ...
	  &CgiDie("cgi_lib.pm: reached end of input while seeking boundary " .
	...
	die "Short Read: wanted $amt, got $got\n" if $errflag;
     ...
     HEAD:
	  ...
	  &CgiDie("cgi_lib: reached end of input while seeking end of " .
...
END_MULTIPART
    ...
    &CgiDie("cgi_lib.pm: Unknown Content-type: $ENV{'CONTENT_TYPE'}\n");
  }
